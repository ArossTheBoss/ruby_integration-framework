...
class ClientAndBrandClient < HttpClient
  ...
  def data(brand_id: nil)
    ...
    {
      'name': 'Test-Client-' + DateTime.now.to_s,
      'notes': 'Test Client',
    ...
    self.put(path: "api/catalog/clients/#{created_client_id}/restrict_client_users", payload: payload)
	end
    ...
    self.post(path:'api/catalog/brands', payload: data)
	end
